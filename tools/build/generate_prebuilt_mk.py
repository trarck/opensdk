...
class PrebuiltMKGenerator(object):
    ...
    def __init__(self, src_mk_path, lib_file_path, dst_mk_path=None,dist_include_path=None):
        ...
        if dist_include_path is None:
            self.dist_include_path = os.path.dirname(self.src_mk_path)
        else:
            if os.path.isabs(dist_include_path):
                self.dist_include_path = dist_include_path
            else:
                self.dist_include_path = os.path.abspath(dist_include_path)
    ...
    def get_lib_file_name(self, lines):
        ...
        for line in lines:
            ...
            match1 = re.match(PrebuiltMKGenerator.LIB_MODULE_FILENAME_PATTERN, trim_line)
            if match1 is not None:
                module_file_name = match1.group(1)

            match2 = re.match(PrebuiltMKGenerator.LIB_MODULE_PATTERN, trim_line)
    ...
    def modidy_src_file(self, lines, new_src_file):
        ...
        for line in lines:
            trim_line = line.lstrip(" ")
            if re.match(PrebuiltMKGenerator.SRC_FILE_CFG_PATTERN, trim_line):
        ...
        if not found_src_file_cfg:
            ...
            for line in new_lines:
                ...
                if re.match(PrebuiltMKGenerator.LIB_MODULE_FILENAME_PATTERN, line):
    ...
    def replace_config(self, lines, cfg_key,target_key):
        new_lines = []
        line_pattern = r"^%s[ ]+[\+\:]=[ ]+.+" % cfg_key
        for line in lines:
            trim_line = line.lstrip(" ")
            ...
            if re.match(line_pattern, trim_line):
                new_line = line.replace(cfg_key,target_key)
    ...
    def modify_config(self, lines, cfg_key):
        new_lines = []
        cfg_begin = False
        
        
        
        #check is not export cfg
        if cfg_key.find("LOCAL_")==0 and cfg_key.find("_EXPORT_")==-1:
            export_cfg_key=cfg_key.replace("LOCAL_","LOCAL_EXPORT_")
            line_pattern = r"^%s[ ]+[\+\:]=[ ]+.+" % export_cfg_key
            #check have export cfg        
            have_export_cfg = False
            for line in lines:
                trim_line = line.lstrip(" ")
                if re.match(line_pattern, trim_line):
                    have_export_cfg = True
                    break            
            if not have_export_cfg:
                return self.replace_config(lines,cfg_key,export_cfg_key)
        
        #remove cfg
        return self.remove_config(lines,cfg_key)
        
    def modify_export_c_include(self, lines):
        #if self.src_mk_path == self.dst_mk_path:
        #    return lines
        dst_dir = os.path.dirname(self.dst_mk_path)
        if dst_dir == self.dist_include_path:
            return lines
        ...
        for line in lines:
            trim_line = line.lstrip(" ")
            match = re.match(PrebuiltMKGenerator.EXPORT_INCLUDE_PATTERN, trim_line)
        ...
        #dst_dir = os.path.dirname(self.dst_mk_path)
        rel_path = os.path.relpath(self.dist_include_path, dst_dir)
        new_include_paths = []
    ...
    def modify_include_cfg(self, lines):
        ...
        for line in lines:
            trim_line = line.lstrip(" ")
            if re.match(PrebuiltMKGenerator.INCLUDE_CFG_PATTERN, trim_line):
    ...
    def modify_import_module(self, lines):
        ...
        for line in lines:
            trim_line = line.lstrip(" ")
            match = re.match(PrebuiltMKGenerator.INCLUDE_MODULE_PATTERN, trim_line)
            if match is not None:
    ...
    def remove_comment(self,origin_lines):
        new_lines = []
        for line in origin_lines:
            trim_line = line.lstrip(" ")
            if len(trim_line)>0 and trim_line[0]!="#" :
                new_lines.append(trim_line)
        return new_lines
    
    def split_modules(self, origin_lines):
        ret = []
        cur_module = {}
        cur_module[PrebuiltMKGenerator.KEY_MODULE_LINES] = []
        cur_module[PrebuiltMKGenerator.KEY_IS_MODULE] = False

        pattern_begin = r'include[ \t]+\$\(CLEAR_VARS\)'
        pattern_end = r'include[ \t]+\$\(BUILD_STATIC_LIBRARY\)'
        for line in origin_lines:
            if re.match(pattern_begin, line):
                if len(cur_module[PrebuiltMKGenerator.KEY_MODULE_LINES]) > 0:
                    ret.append(cur_module)

                cur_module = {}
                cur_module[PrebuiltMKGenerator.KEY_MODULE_LINES] = []
                cur_module[PrebuiltMKGenerator.KEY_IS_MODULE] = True

            cur_module[PrebuiltMKGenerator.KEY_MODULE_LINES].append(line)
    ...
    def handle_module(self, module_lines, relative_path):
        ...
        dst_lines = self.modify_config(dst_lines, "LOCAL_C_INCLUDES")
        dst_lines = self.modify_config(dst_lines, "LOCAL_LDLIBS")
    ...
    def do_generate(self):
        ...
        src_lines = self.remove_comment(src_lines)
        modules = self.split_modules(src_lines)
        dst_lines = []
        for module in modules:
            if module[PrebuiltMKGenerator.KEY_IS_MODULE]:
                ret_lines = self.handle_module(module[PrebuiltMKGenerator.KEY_MODULE_LINES], relative_path)
            else:
                ret_lines = module[PrebuiltMKGenerator.KEY_MODULE_LINES]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dist-include-path", dest='dist_include_path', help='Default is src mk file dir.If keep the include path of src file,the value set to dest mk dir.')
    (args, unknown) = parser.parse_known_args()

    gen_obj = PrebuiltMKGenerator(args.src_mk, args.lib_path, args.dst_mk,args.dist_include_path)
    gen_obj.do_generate()
